...
class FatigueModel: ObservableObject {
    ...
    func getFatigueScore() {
        // Calculate the fatigue score asynchronously
        calculator.CalculateScore { [weak self] in
            guard let self = self else { return }
            
            DispatchQueue.main.async {
                // Update fatigue score
                self.fatigueScore = self.calculator.fatigueScore
                
                // Update all metric strings for UI display
                self.setSleepString()
                self.setRestingHRString()
                self.setStepsString()
                self.setCaloriesString()
                
                // Trigger notification if fatigue is high
                if self.fatigueScore > 80 {
